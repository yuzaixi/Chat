...
import com.huidiandian.meeting.base.constant.LiveDataBusConstantBaseModule.ON_PUBLISH
import com.huidiandian.meeting.base.constant.LiveDataBusConstantBaseModule.ON_STOP_PUBLISH
import com.huidiandian.meeting.base.eventmsg.room.webrtc.OnPublishMsg
...
class ScreenShareViewModel : ViewModel() {
     ...
     * 处理发布事件的观察者
     */
    private val onPublishObserver =
        Observer<OnPublishMsg> { message ->
            AppLogger.getInstance().d("$TAG 收到发布事件: $message")
            if (message.streamType != QStreamType.SHARE) {
                AppLogger.getInstance().d("$TAG 忽略非共享流的发布事件: ${message.streamType}")
                return@Observer
            }
            if (message.account != account) {
                AppLogger.getInstance().d("$TAG 忽略非当前账号的发布事件: ${message.account}")
                return@Observer
            }
            viewModelScope.launch(Dispatchers.IO) {
                val isPullStream = QRtcEngine.get().checkVideoPull(
     ...
     * 处理停止发布事件的观察者
     */
    private val onStopPublishObserver =
        Observer<OnPublishMsg> { message ->
            AppLogger.getInstance().d("$TAG 收到停止发布事件: $message")
            if (message.streamType != QStreamType.SHARE) {
                AppLogger.getInstance().d("$TAG 忽略非共享流的发布事件: ${message.streamType}")
                return@Observer
            }
    ...
    private fun setupEventHandlers() {
        ...
        LiveEventBus.get(ON_PUBLISH, OnPublishMsg::class.java)
            .observeForever(onPublishObserver)
        LiveEventBus.get(ON_STOP_PUBLISH, OnPublishMsg::class.java)
            .observeForever(onStopPublishObserver)
    ...
    private fun removeEventHandlers() {
        ...
        LiveEventBus.get(ON_PUBLISH, OnPublishMsg::class.java)
            .removeObserver(onPublishObserver)
        LiveEventBus.get(ON_STOP_PUBLISH, OnPublishMsg::class.java)
            .removeObserver(onStopPublishObserver)
