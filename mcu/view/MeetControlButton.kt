...
class MeetControlButton @JvmOverloads constructor(
    ...
    fun setButtonImage(resId: Int) {
        binding.btnIcon.setImageResource(resId)
    ...
    private fun updateMicClipDrawableIfNeeded(resId: Int) {
        if (resId == R.drawable.layer_icon_mic_opened) {
            val drawable = binding.btnIcon.drawable
    ...
    fun showMemberNumbers(numbers: Int) {
        binding.tvNumber.text = if (numbers < 99) numbers.toString() else "99+"
        binding.tvNumber.visibility = if (numbers > 0) VISIBLE else GONE
    }
