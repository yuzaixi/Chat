...
import com.huidiandian.meeting.base.appupdate.Constant.IM_MSG_TYPE
import com.huidiandian.meeting.base.client.arguments.EnMessageType
...
import com.huidiandian.meeting.base.eventmsg.room.webrtc.OnIMMessage
import com.huidiandian.meeting.base.eventmsg.room.webrtc.OnSetRollCallStatus
import com.huidiandian.meeting.base.eventmsg.room.webrtc.RoomApplyMessgae
...
class MCUMeetActivity : MVVMActivity<ActivityMcuMeetBinding, MCUMeetViewModel>() {
    ...
    private fun setupGestureHandler() {
        ...
        gestureDetector = GestureDetector(this, object : GestureDetector.SimpleOnGestureListener() {
            override fun onSingleTapConfirmed(e: MotionEvent): Boolean {
                AppLogger.getInstance().d("单击事件")
                if (meetHostFragment?.isVisible == true) return false // 如果参会者列表可见，忽略单击事件
                // 判断是否在菜单区域，避免误响应
                if (binding.meetMenuView.isPointInMenuArea(e.x, e.y)) return false
                if (binding.applyShowView.isPointInApplyArea(e.x, e.y)) return false
                handleSingleTap()
                return true
            }
    ...
    private fun handleSingleTap() {
        ...
        if (now - lastSingleClickTime > CLICK_THRESHOLD) {
    ...
    private fun handleDoubleTap(e: MotionEvent) {
        val mainFragment = getCurrentMainFragment() ?: return
        val layoutView = mainFragment.getMcuLayoutView() ?: return
        AppLogger.getInstance().d("双击事件处理 ; isFullScreen: ${mainFragment.isFullScreen()}")
        if (mainFragment.isFullScreen()) {
            // 全屏状态下，任何位置双击都直接退出全屏
            mainFragment.exitFullScreen()
        } else {
            // 非全屏，查找命中用户
            val (userId, streamType) = layoutView.findUserAt(e.x, e.y)
            if (!userId.isNullOrEmpty()) {
                mainFragment.enterFullScreen(userId, streamType)
            }
        }
    }
    ...
    private fun setupStatusBar() {
        logD("设置状态栏")
        ImmersionBar.with(this).transparentStatusBar() // 设置状态栏透明
            .init() // 初始化
    }
    ...
    private fun finishAndJumpStatistics() {
        ...
        ARouter.getInstance().build(RouterUrlManager.MEETING_DURATION_STATISTICS_ACTIVITY).withLong(
            IntentValues.MEETING_CREATE_TIMES, state.meetingInfo?.startTimeMs ?: 0
        ) // 会议创建时间
            .withString(
                IntentValues.MEETING_DURATION, TimeUtil.secondToHHmmss(duration)
            ) // 会议时长（格式化）
    ...
    private fun showLeaveDialogByParticipant() {
        ...
        DialogUtils.showCommonDoubleBtnNewStyleDialog(
            this, this.getString(R.string.meeting_str_sure_to_leave_meeting), // 提示文本
    ...
    private fun showMicBanDialog() {
        ...
        val hostBanMicDialog = DialogHelper.getCommonDialog(
            this, hostBanLoudSpeakerView, Gravity.CENTER, // 居中显示
            R.style.center_alpha_scale_animation, // 动画样式
            width, height, 0.8f // 背景透明度
        )
        // 获取布局中的按钮
    ...
    private fun setupObservers() {
        ...
        observeState({ it.userInfo }) { userInfoState ->
            ...
            localFloatViewManager?.updateUserInfo(
                userInfoState?.userInfo?.name, userInfoState?.userInfo?.icon
            )
        }
        // 观察会议信息变化
        ...
        observeState({
            SharingState(
                it.isSelfSharing, it.shareAccount, it.isSharing
            )
        }) { sharingState ->
        ...
        observeState({ it.roomCloseReason }) {
            AppLogger.getInstance().d("观察到会议关闭原因变化: $it")
            if (it == -1) return@observeState
            finishAndJumpStatistics()
        }
        observeState({ it.memberNumbers }) {
            AppLogger.getInstance().d("观察到参会者人数变化: $it")
            binding.meetMenuView.showNumbers(it) // 更新菜单栏参会者人数
        }
        // 观察本地麦克风音量变化 (假设 localMicVolume 是 Flow)
        lifecycleScope.launch {
            viewModel.localMicVolume.flowWithLifecycle(lifecycle, Lifecycle.State.STARTED) // 绑定生命周期
                .collect { volume ->
        ...
        LiveEventBus.get(LiveDataBusConstantBaseModule.ON_IM_MESSAGE, OnIMMessage::class.java)
            .observe(this) {
                val msg = RoomApplyMessgae(
                    it.message?.sender?.userInfo?.account.toString(),
                    it.message?.sender?.userInfo?.name.toString(),
                    IM_MSG_TYPE
                )
                msg.content = it.message?.content.toString()
                binding.applyShowView.addApply(msg)
            }
        LiveEventBus.get(LiveDataBusConstantBaseModule.ON_MESSAGE, RoomApplyMessgae::class.java)
            .observe(this) {
                val role = viewModel.viewState.value.role
                if (role == QRoomRole.HOST) {
                    if (it.messageType != EnMessageType.MESSAGE_START_RECORD && it.messageType != EnMessageType.MESSAGE_HOST_LEFT) {
                        //主持人显示参会者申请消息
                        binding.applyShowView.addApply(it)
                    }
                } else {
                    //参会者显示主持人已开启录制，主持人离会消息
                    if (it.messageType == EnMessageType.MESSAGE_START_RECORD || it.messageType == EnMessageType.MESSAGE_HOST_LEFT || it.messageType == EnMessageType.MESSAGE_PARTICIPANT_LEFT || it.messageType == EnMessageType.MESSAGE_PARTICIPANT_JOIN) {
                        binding.applyShowView.addApply(it)
                    }
                }
            }
        LiveEventBus.get(
            LiveDataBusConstantBaseModule.ON_SET_ROLL_CALL_STATUS,
            OnSetRollCallStatus::class.java
        )
            .observe(this) {
                if (it.lecturerStatus.not()) {
                    binding.applyShowView.setSpeakerAccount("")
                } else {
                    binding.applyShowView.setSpeakerAccount(it.newLecturer)
                }
                binding.applyShowView.setSpeakStatus(it.lecturerStatus)
            }
        // 监听请求隐藏参会者列表的事件
    ...
    private fun updateSharingState(state: SharingState) {
        binding.meetMenuView.updateSharingState(state.isSelfSharing) // 更新菜单栏共享按钮状态
        // 如果是自己正在共享，显示顶部的停止共享控制条
        binding.layoutShareLayer.root.visibility = if (state.isSelfSharing) VISIBLE else GONE
    ...
    private fun isInFullScreenMode(): Boolean {
        return getCurrentMainFragment()?.isFullScreen() == true
    }
    ...
    private fun safelyReleaseResources() {
        ...
        try {
            logD("开始安全释放资源")
            if (null != startPublishParams) {
                startPublishParams?.callBack = null
            }
            // 解绑屏幕共享服务
            if (mScreenSharingBinder != null) {
                ...
                val service = Intent()
                service.setClass(
                    MiguVideoApplicaition.getContext(), QScreenSharingService::class.java
                )
                stopService(service)
                mScreenSharingBinder?.release()
                mScreenSharingBinder?.screenSharingListener = null
                mScreenSharingBinder = null
                QScreenSharingService.serviceChannel = null
            } else {
                logD("屏幕共享服务未绑定或已解绑")
            }
    ...
    private fun initNetAndStreamStatePop() {
        ...
        if (netAndStreamStatePop == null) { // 仅在实例为空时创建
            ...
            netAndStreamStatePop?.dismissBlock = {
                ...
                viewModel.setStreamStatsInterval(10)
            }
    ...
    private fun appendInviteInfoContent(toastMsg: String) {
        ...
        stringBuilder.append(
            resources.getString(
                R.string.str_room_number_with, roomInfo?.room?.id ?: "N/A"
            ) + "\n" // 会议号
        )
