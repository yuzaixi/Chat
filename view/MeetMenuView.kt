...
import com.bifrost.rtcengine.constants.QStreamQualityLevel
...
import com.huidiandian.meeting.meeting.event.NetWorkStateChanged
...
import com.huidiandian.meeting.meeting.room.SignalMeeting
import com.huidiandian.meeting.meeting.room.meethost.InviteCallListPopWindow
import com.huidiandian.meeting.meeting.room.playinfo.PlayStreamQualityInfo
...
class MeetMenuView @JvmOverloads constructor(
    ...
    private fun initMenuEventListeners() = with(binding) {
        val clickActions = mapOf(
            ...
            netIntensity to { menuItemClickListener?.onNetIntensityClick() },
        )
    ...
    private fun invite() {
        if (roomInfo?.meetingLock == true) {
            DialogUtils.showCommonDoubleBtnDialog(
                context,
                context.getString(R.string.meeting_str_lock_hint),
    ...
    fun updateSharingState(sharing: Boolean) = with(binding.menuShare) {
        ...
        if (sharing) {
            ...
            setButtonTextColor(resources.getColor(com.huidiandian.meeting.base.R.color.white))
        }
    }

    suspend fun updateMeetSignal(
        serverDelay: Int?,
        pushMyVideoStreamQuality: QStreamQualityLevel?,
        badPlayInfo: List<PlayStreamQualityInfo>?,
    ) {
        val signalLevel = calculateSignalLevel(serverDelay, pushMyVideoStreamQuality, badPlayInfo)
        withContext(Dispatchers.Main) {
            updateNetworkSignalUI(signalLevel)
            LiveEventBus.get(
                LiveDataBusConstantBaseModule.NET_WORK_CHANGE,
                NetWorkStateChanged::class.java
            ).post(NetWorkStateChanged(signalLevel))
            val netType = NetWorkUtil.getNetTypeValue(NetWorkUtil.getNetworkType(context))
            if (netType == NetWorkUtil.NETWORK_NO_NET_VALUE) {
                binding.netIntensity.setImageResource(R.drawable.meeting_icon_wifi_zero)
            }
        }
    }

    private fun calculateSignalLevel(
        serverDelay: Int?,
        pushMyVideoStreamQuality: QStreamQualityLevel?,
        badPlayInfo: List<PlayStreamQualityInfo>?,
    ): Int {
        badPlayInfo?.forEach {
            if (it.quality.level == QStreamQualityLevel.DIE) return SignalMeeting.DIE
            if (it.quality.level == QStreamQualityLevel.BAD) return SignalMeeting.BAD
        }
        pushMyVideoStreamQuality?.let {
            if (it == QStreamQualityLevel.BAD) return SignalMeeting.BAD
            if (it == QStreamQualityLevel.DIE) return SignalMeeting.DIE
        }
        return calculateDelaySignal(serverDelay)
    }

    private fun calculateDelaySignal(serverDelay: Int?): Int {
        if (serverDelay == null) return SignalMeeting.MEDIUM
        return when {
            serverDelay <= 50 -> SignalMeeting.EXCELLENT
            serverDelay <= 100 -> SignalMeeting.GOOD
            serverDelay <= 200 -> SignalMeeting.MEDIUM
            serverDelay <= 400 -> SignalMeeting.BAD
            else -> SignalMeeting.DIE
        }
    }

    private fun updateNetworkSignalUI(signalLevel: Int) {
        val iconRes = when (signalLevel) {
            SignalMeeting.EXCELLENT -> R.drawable.meeting_icon_wifi_five
            SignalMeeting.GOOD -> R.drawable.meeting_icon_wifi_four
            SignalMeeting.MEDIUM -> R.drawable.meeting_icon_wifi_three
            SignalMeeting.BAD -> R.drawable.meeting_icon_wifi_two
            SignalMeeting.DIE -> R.drawable.meeting_icon_wifi_one
            else -> R.drawable.meeting_icon_wifi_three
        }
        binding.netIntensity.setImageResource(iconRes)
    }
